...
class BaseSSL(nn.Module):
    ...
    IMAGENET_PATH = os.environ.get('IMAGENET_PATH', './models/data/imagenet/raw-data/')
    ...
    def prepare_data(self):
        ...
        if self.hparams.data == 'cifar':
        ...
        elif self.hparams.data == 'imagenet100':
            traindir = os.path.join('./models/data/imagenet100/raw-data/', 'train')
            valdir = os.path.join('./models/data/imagenet100/raw-data/', 'val')
            self.trainset = datasets.ImageFolder(traindir, transform=train_transform)
            self.testset = datasets.ImageFolder(valdir, transform=test_transform)
            ...
            # 在这里添加 STL-10
            self.trainset = datasets.STL10(
                root=self.DATA_ROOT, split='train', download=True, transform=train_transform
            )
            self.testset = datasets.STL10(
                root=self.DATA_ROOT, split='test', download=True, transform=test_transform
            )

        elif self.hparams.data == 'stl10_unlabeled':
            self.trainset = datasets.STL10(
                root=self.DATA_ROOT, split='unlabeled', download=True, transform=train_transform
            )
            self.testset = datasets.STL10(
                root=self.DATA_ROOT, split='test', download=True, transform=test_transform
            )
...
class SimCLR(BaseSSL):
    ...
    def __init__(self, hparams, device=None):
        ...
        else:# InfoNCE variants
            self.criterion = models.losses.gcl_IS(
                tau=hparams.temperature,
                multiplier=hparams.multiplier,
                alpha = hparams.alpha,
                beta=hparams.beta,
    ...
    def step(self, batch):
        ...
        if self.loss_type==2: #独立采样
            loss, acc = self.criterion.forward_IS(z)

        elif self.loss_type==3: #DCL
            loss, acc = self.criterion.forward_DCL(z)

        elif self.loss_type==4: #DHEL
            loss, acc = self.criterion.forward_DHEL(z)

        elif self.loss_type==5: #KCL-Gaussian
            loss, acc = self.criterion.forward_GKCL(z)
    ...
    def transforms(self):
        ...
        elif self.hparams.data == 'imagenet' or self.hparams.data == 'imagenet100':
        ...
        elif 'stl10' in  self.hparams.data:
            # 这里自定义 STL-10 的图像大小和增强
            # from utils.datautils import GaussianBlur
            im_size = 96  # STL-10 图像分辨率为 96×96
            train_transform = transforms.Compose([
                transforms.RandomResizedCrop(
                    im_size,
                    scale=(self.hparams.scale_lower, 1.0),
...
class SSLEval(BaseSSL):
    ...
    def __init__(self, hparams, device=None):
        ...
        if hparams.encoder_ckpt != '':
            ckpt = torch.load(hparams.encoder_ckpt, map_location=device, weights_only=False)
        ...
        elif hparams.data == 'stl10':
            hdim = self.encode(torch.ones(10, 3, 96, 96).to(device)).shape[1]
            n_classes = 10
    ...
    def step(self, batch):
        if self.hparams.problem == 'eval' and 'imagenet' in self.hparams.data:
            batch[0] = batch[0] / 255.
        h, y = batch
    ...
    def prepare_data(self):
        ...
        def create_emb_dataset(dataset):
            ...
            for x, y in tqdm(loader):
                if 'imagenet' in self.hparams.data:
    ...
    def transforms(self):
        if  'cifar' in self.hparams.data:
            ...
            if 'RandomCrop' in self.hparams.augmentation:
                trs.append(transforms.RandomCrop(96, padding=4, padding_mode='reflect'))
            if 'color_distortion' in self.hparams.augmentation:
                trs.append(datautils.get_color_distortion(self.encoder.hparams.color_dist_s))
        ...
        elif 'imagenet' in self.hparams.data:
            train_transform = transforms.Compose([
                transforms.RandomResizedCrop(
                    224,
                    scale=(self.hparams.scale_lower, 1.0),
